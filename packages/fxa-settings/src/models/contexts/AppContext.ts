...
import {
  ...
  getUUID,
  createUUID,
  forceCurrentAccountFromUidQueryParams,
  purgeInvalidAccounts,
} from '../../lib/cache';
    ...
    if (resumeToken) {
      try {
        ...
        if (typeof resumeTokenObj?.uniqueUserId === 'string') {
          setUUID(resumeTokenObj.uniqueUserId);
...
export function initializeAppContext() {
  ...
  const currentAccountUid = forceCurrentAccountFromUidQueryParams();
